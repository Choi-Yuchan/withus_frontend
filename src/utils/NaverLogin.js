...
export const NaverLogin = ({ setGetToken, setUserInfo }) => {
  console.log(setGetToken, setUserInfo);
  const naverRef = useRef();
  const { naver } = window;
  const NAVER_CLIENT_ID = process.env.REACT_APP_NAVER_CLIENT_ID; // 발급 받은 Client ID 입력
  const NAVER_CALLBACK_URL = "http://3.36.254.118:8181/login/oauth2/code/naver";
  // "http://3.36.254.118:8181/login/oauth2/code/naver";
  ...
  return (
    <>
      ...
      <NaverLoginBtn id="naverIdLogin" ref={naverRef} />
      <NaverBtn onClick={(e) => handleNaverLogin(e)}>
        네이버 아이디로 로그인
      </NaverBtn>
    </>
  );
